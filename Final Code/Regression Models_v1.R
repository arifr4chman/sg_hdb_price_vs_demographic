...
# Visualise regression lines using linear model
model_lnr_price_lg_lnr_5a <- lm(price ~ y30_34Years*remaining_lease, 
                                data=hdb_final)
summary(model_lnr_price_lg_lnr_5a)

model_lnr_price_lg_lnr_5b <- lm(price ~ y80_84Years*remaining_lease, 
                                data=hdb_final)

par(mfrow=c(1,2))
reg_line1 <- ggplot(hdb_final, aes(y=price, x=y30_34Years)) + geom_point() +
  geom_smooth(method = "lm")
reg_line1

reg_line2 <- ggplot(hdb_final, aes(y=price, x=y80_84Years)) + geom_point() +
  geom_smooth(method = "lm")
reg_line2

ggPredict(model_lnr_price_lg_lnr_5a, se = TRUE, interactive = FALSE)
ggPredict(model_lnr_price_lg_lnr_5b, se = TRUE, interactive = FALSE)
